...
- 대기중/상담중/창구 안내/돌려보냄 상태와 예약 확인값(뒤 4자리·별칭 권장), 응대 창구/담당자 기록
...
- **현장 현황**: 대기중/상담중/창구 안내 상태, 예약 확인값, 응대 창구/담당자를 한눈에 확인
...
- **JSON/CSV 내보내기 · JSON 가져오기**(내보내기 전 재스캔, 예약 확인값 원문은 내보내기에서 삭제)
...
- **응대 시간 자동/수동 기재**: 실제 사건 시각이 아닌 소요 시간(분/초)만 기록. 자동 모드는 빠른 기록
  폼을 여는 순간부터 타이머로 측정하고, 수동 모드는 분/초를 직접 입력. 과거 로그를 편집할 때는 이미
  지난 시점을 재측정할 수 없으므로 자동 옵션을 숨기고 수동 조정만 허용한다.
